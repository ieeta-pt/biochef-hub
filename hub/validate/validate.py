...
def validate_source_identity(field, value, error):
    ...
    if tag and not repo:
        error(field, "source.tag is only valid for git sources with source.repo")
    ...
    for candidate in [repo]:
        ...
        if (
            ...
            error(field, "git source repositories must be credential-free HTTPS URLs")
...
def validate_spdx_license_expression(field, value, error):
    try:
        expression = spdx_licensing.parse(value, validate=False)
    ...
    unknown = [
        ...
        if symbol.key not in spdx_licensing.known_symbols
        and not license_ref.fullmatch(symbol.key)
    ]
    if unknown:
        error(field, f"unknown SPDX license identifier(s): {unknown}")
