...
def generate_sign_and_publish_catalog(
    ...
    packages = [artifact["package"] for artifact in publish_results["artifacts"]]
...
def _publish_catalog(registry_url: str, catalog_path: Path, signature_path: Path, package_prefix: str, catalog_version: str, channel: str) -> dict[str, Any]:
    ...
    if version_digest != latest_digest:
        raise CatalogError(
            "Registry returned different catalog manifests for version and channel tags"
        )
    return {
...
def _catalog_version_from_environment() -> str:
    ...
    if run_id and sha:
        attempt = run_attempt or "1"
        return f"catalog-{run_id}-{attempt}-{sha[:12]}"
    return f"catalog-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}"
